...
| `sensor.*_action_latency` | Measurement (ms) | Wall-clock time of the last charger action call in milliseconds, including any retries. Cross-reference with `retry_count` to distinguish slow actions from retried ones. |
